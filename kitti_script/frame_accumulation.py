...
def accumulate_sequence(root_path, seq, Tr,
                        ...
                        is_plot=True, P_num = 'P2'):
    np_folder = 'voxel0.1-SNr0.6'
    output_folder = 'stride%d-acc%d-voxel%.1f' % (frame_stride_dist, accumulate_radius,voxel_size)
    output_folder_path = os.path.join(root_path, 'output','data_odometry_velodyne_%s_NWU' % P_num, 'sequences',
    ...
    pc_nwu_folder = os.path.join(root_path, 'output', 'data_odometry_velodyne_%s_npy' % P_num, 'sequences', '%02d' % seq, np_folder)
    ...
    stride_list = [0]
    P_prev = np.load(os.path.join(pose_folder, '%06d.npy' % 0))  # 4x4
    for i in range(1, sample_num):
        P_i = np.load(os.path.join(pose_folder, '%06d.npy' % i))  # 4x4
    ...
    for i in range(0, sample_num):
        Pi = np.load(os.path.join(pose_folder, '%06d.npy' % i))  # 4x4
        ...
        for stride in stride_list[stride_list_left_idx:stride_list_right_idx+1]:
            Pj = np.load(os.path.join(pose_folder, '%06d.npy' % stride))  # 4x4
