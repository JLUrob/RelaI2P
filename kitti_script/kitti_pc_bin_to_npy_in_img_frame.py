        ...
        for i in range(sample_num):
            ...
            '''open3d.geometry.estimate_normals(downpcd,
                                             search_param=open3d.geometry.KDTreeSearchParamHybrid(radius=sn_radius,
                                                                                                    max_nn=sn_max_nn))'''
            downpcd.estimate_normals(search_param=open3d.geometry.KDTreeSearchParamHybrid(radius=sn_radius,
                                                                                                    max_nn=sn_max_nn))
            ...
            for img_key in ['image_2', 'image_3']:
                ...
                pc_down_img_np = calib_helper.transform_pc_vel_to_img(pc_down_np, seq, img_key_P) #并非是像素坐标系，而是未经过内参的相机坐标系 
                ...
                # ------------------------------
                img_folder = os.path.join(root_path, 'sequences', '%02d' % seq, img_key)
                K = calib_helper.get_matrix(seq, img_key_P + '_K')  # 3x3

                # load image
                img_path = os.path.join(img_folder, '%06d.png' % i)
                img = cv2.imread(img_path)
                # img = np.load(img_path)
                H, W = img.shape[0], img.shape[1]
                img_depth = np.zeros((H, W), dtype=np.float32) # 保存尺度因子（保留了最小尺度因子）
                ...
                KP_pc_homo = np.dot(K, P_pc_homo)  # 3xN
                KP_pc_pxpy = KP_pc_homo[0:2, :] / KP_pc_homo[2:3, :]  # 2xN #保存点云mask
                

                for j in range(KP_pc_pxpy.shape[1]):
                    px = KP_pc_pxpy[0, j]
                    py = KP_pc_pxpy[1, j]
                    px_round = int(round(px))#四舍五入
                    py_round = int(round(py))#四舍五入
                    pz = P_pc_homo[2, j]
                    ...
                    if pz>0.1 and px_round >= 0 and px_round <= W-1 and py_round >= 0 and py_round <= H-1:
                        ...
                        if dist_to_nn_pc < img_depth_dist_to_nn_pc[py_round, px_round]:
                            img_depth[py_round, px_round] = pz # 尺度因子
                            img_depth_dist_to_nn_pc[py_round, px_round] = dist_to_nn_pc #保存最近位置的点云，因为会出现多个点云打在同一个像素上的问题，则保留最近的
                ...
                edt_val, edt_idx = distance_transform_edt(img_depth_mask_inv, return_indices=True, return_distances=True)# 计算二值图像每个非零像素点(点云未达到)到零像素点（点云，零是因为上个函数反转了）的最短距离，并返回其距离和最短零像素坐标索引

                img_depth_dist_to_nn_pc = img_depth_mask * img_depth_dist_to_nn_pc + img_depth_mask_inv * edt_val # 点云未打中的像素点用最近打中像素点的欧式距离表示（像素欧式距离），打中的像素点则使用最近的（像素欧式距离）
                img_depth = img_depth[edt_idx[0], edt_idx[1]] # 点云未打中的点用最近点云点的尺度因子

                img_depth_output = np.stack((img_depth, img_depth_dist_to_nn_pc), axis=0).astype(np.float32)  # 2xHxW

                # save to disk
                img_depth_folder = os.path.join(root_path, 'output', 'data_odometry_color_depth_npy', 'sequences', '%02d' % seq, img_key)
...
if __name__ == '__main__':
    # root_path = 'F:\\KITTI\\odometry\\dataset'
    # pc_bin_root_path = 'F:\\KITTI\\odometry\\dataset'
    root_path = '../datas/kitti'
    pc_bin_root_path = '../datas/kitti'
    
    # seq_list = list(range(11))
    seq_list = list(range(5,11))
    # print(seq_list)
