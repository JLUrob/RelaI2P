...
class ImageEncoder(nn.Module):
    def __init__(self, model):
        ...
        if model =='pcd_360_img':
            self.backbone = resnet50(in_channels=8, pretrained=False, progress=True)
        # image mesh grid
...
class Image_ResNet(nn.Module):
    def __init__(self, model):
        ...
        self.name = model
        self.encoder = ImageEncoder(self.name)
        
        if self.name == 'img':
            self.up_conv1=ImageUpSample(768,256)
            self.up_conv2=ImageUpSample(256+128,128)
            self.up_conv3=ImageUpSample(128+64+64,64)
        if self.name == 'pcd_360_img':
            self.up_conv1=ImageUpSample(2048+1024,1024)
            self.up_conv2=ImageUpSample(1024+512,512)
            self.up_conv3=ImageUpSample(512+256+64,256)
    ...
    def forward(self, x):
        ...
        # if self.name == 'pcd_360_img':
        # if self.name == 'img':
        #     print(img_feature_set[-1].shape)
        #     print(img_feature_set[-2].shape)
        #     print(img_feature_set[-3].shape)
        #     print(img_feature_set[-4].shape)
        #     print(img_feature_set[-5].shape)
        #     print(img_feature_set[-6].shape)
        #     while(1):pass
        img_global_feature=img_feature_set[-1]  # resnet34：512       resnet50:2048
        img_s32_feature_map=img_feature_set[-2] #           512                2048
        img_s16_feature_map=img_feature_set[-3] #           256                1024
        img_s8_feature_map=img_feature_set[-4]  #           128                512
        img_s4_feature_map=img_feature_set[-5]  #           64                 256
        img_s2_feature_map=img_feature_set[-6]  #           64                 64
        ...
        return img_global_feature.squeeze(-1).squeeze(-1), image_feature_mid, img_s16_feature_map, img_s2_feature_map
