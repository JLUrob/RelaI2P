...
from models_corr_360.pointTransformer.point_transformer import PointsEncoder_pointwise
from models_corr_360.network_img.resnet import Image_ResNet
from models_corr_360.spvnas.models.semantic_kitti.spvcnn import SPVCNN
from models_corr_360.kitti_dataset import tensor_generate_gaussian_distribution, generate_gaussian_distribution
from utils.options import Options
from utils.dimtrans import ChannelTransformer
...
class VP2PMatchNet(nn.Module):
    ...
    # def forward(self,pc,intensity,sn,img,pcd_360_img):
        ...
        global_img_feat, pixel_wised_feat, pixel_wised_feat_0, pixel_wised_feat_1 = self.resnet(img)
        global_pcd_360_feat, pcd_360_feat, pcd_360_feat_0, pcd_360_feat_1 = self.pcd_img_resnet(pcd_360_img)
        
        # ----------------------通道变换---------------------
        if pixel_wised_feat_0.shape[1] != pcd_360_feat_0.shape[1]:
            Channeltrans_0 = ChannelTransformer(max(pixel_wised_feat_0.shape[1], pcd_360_feat_0.shape[1]), min(pixel_wised_feat_0.shape[1], pcd_360_feat_0.shape[1])).cuda()
        if pixel_wised_feat_1.shape[1] != pcd_360_feat_1.shape[1]:
            Channeltrans_1 = ChannelTransformer(max(pixel_wised_feat_1.shape[1], pcd_360_feat_1.shape[1]), min(pixel_wised_feat_1.shape[1], pcd_360_feat_1.shape[1])).cuda()
        feat_map = {}
        if pixel_wised_feat_0.shape[1]>pcd_360_feat_0.shape[1]:
            pixel_wised_feat_0 = Channeltrans_0(pixel_wised_feat_0)
        if pixel_wised_feat_0.shape[1]<pcd_360_feat_0.shape[1]:
            pcd_360_feat_0 = Channeltrans_0(pcd_360_feat_0)
        if pixel_wised_feat_1.shape[1]>pcd_360_feat_1.shape[1]:
            pixel_wised_feat_1 = Channeltrans_1(pixel_wised_feat_1)
        if pixel_wised_feat_1.shape[1]<pcd_360_feat_1.shape[1]:
            pcd_360_feat_1 = Channeltrans_1(pcd_360_feat_1)
        feat_map["pixel_feat_0"] = pixel_wised_feat_0
        feat_map["pcd_360_feat_0"] = pcd_360_feat_0
        feat_map["pixel_feat_1"] = pixel_wised_feat_1
        feat_map["pcd_360_feat_1"] = pcd_360_feat_1
        # --------------------------------------------------
        ...
        return pixel_wised_feat, point_wised_feat , img_score, pc_score, pre_dist, feat_map
...
if __name__=='__main__':
    ...
    a,b,c,d,e,m=net(pc,intensity,sn,img,pcd_360_img,gt_distribution)
    ...
    print(d.size())
    print(e.size())
    print(m["pixel_feat_0"].shape)
    print(m["pcd_360_feat_0"].shape)
    print(m["pixel_feat_1"].shape)
    print(m["pcd_360_feat_1"].shape)
