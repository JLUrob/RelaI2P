...
import torch
import argparse
from models360.triplet_network import VP2PMatchNet
from models360.kitti_dataset import kitti_pc_img_dataset
...
def test_acc_trans(model,testdataloader,args,logger):
    ...
    for step,data in enumerate(testdataloader):
        if step%1==0:
            ...
            img_features_all,pc_features_all,img_score_all,pc_score_all,pre_dist=model(pc_all,intensity,sn,img,pcd_360_img,gt_dist)     #64 channels feature
            
            bs = img_score_all.shape[0]

            for i in range(bs):
                img_score=img_score_all[i]      #(1,40,128)
                pc_score=pc_score_all[i]        #(1,40960)
                img_feature=img_features_all[i] #(64,40,128)
                pc_feature=pc_features_all[i]   #(64,40960)
                pc=pc_all[i]
                ...
                # print(img_x.shape)    #(1,40,128)
                # print(img_y.shape)    #(1,40,128)
                # print('img_x:',img_x)
                # print('img_y:',img_y)
                # while(1): pass
                img_xy=np.concatenate((img_x,img_y),axis=0) #(2,40,128)
                # print(img_xy.shape)
                # print('img_xy:',img_xy)
                # while(1): pass
                ...
                # print(img_score_flatten.shape)
                # print('img_score_flatten:',img_score_flatten)
                # while(1): pass
                ...
                # print(pc_index)
                # print(img_index.shape, pc_index.shape,pc.shape,pc_feature.shape)
                # while(1): pass
            ...
            if step % 100 == 0:
                ...
                logger.info('step:{} --- Good rate :  {} / {}'.format(step, t_diff_set_np[index].shape, t_diff_set_np.shape))
                logger.info('RTE mean {} std {}'.format(np.mean(t_diff_set_np), np.std(t_diff_set_np)))
                logger.info('RRE mean {} std {}'.format(np.mean(angles_diff_set_np), np.std(angles_diff_set_np)))
    ...
    logger.info('Good rate : {} / {}'.format(t_diff_set[index].shape, t_diff_set.shape))
    logger.info('RTE mean {} std {}'.format(np.mean(t_diff_set), np.std(t_diff_set)))
    logger.info('RRE mean {} std {}'.format(np.mean(angles_diff_set), np.std(angles_diff_set)))
...
def vis_registration(model,test_dataset,args):
    ...
    for i in range(20):
        ...
        gt_dist = data['gt_distribution'].cuda().unsqueeze(0)
        pcd_360_img=data['pcd_360_img'].cuda().unsqueeze(0)

        img_features_all,pc_features_all,img_score_all,pc_score_all,pre_dist=model(pc_all,intensity,sn,img,pcd_360_img,gt_dist)     #64 channels feature
        # if(img_features_all.shape[0]==1):
        #     pc_features_all = pc_features_all.unsqueeze(0)
...
if __name__=='__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/home/jlurobot/wzy/Calibration/Lidar2Camera/VP2P-Match-main/data/kitti', metavar='data_path',
                        help='train and test data path')
    parser.add_argument('--num_workers', type=int, default=4, metavar='num_workers',
                        help='num of CPUs')
    parser.add_argument('--input_pt_num', type=int, default=40960, metavar='num_workers',
                        help='num of CPUs')
    # parser.add_argument('--input_pt_num', type=int, default=20480, metavar='num_workers',
    #                     help='num of CPUs')
    ...
    parser.add_argument('--save_path', type=str, default='./kitti_outs', metavar='save_path',
                        help='path to save log and model')
    parser.add_argument('--exp_name', type=str, default='test', metavar='save_path',
                    help='path to save log and model')
    parser.add_argument('--exp_name', type=str, default='360funsion_iter25_lr0.001_input_pt_num_61440', metavar='save_path',
                    help='path to save log and model')
    ...
    parser.add_argument('--neg_margin', type=float, default=1.8, metavar='neg_margin',
                        help='')
    parser.add_argument('--load_ckpt', type=str, default='./kitti_res/360funsion_iter25_lr0.002_input_pt_num_20480/mode_epoch_24.t7', metavar='save_path',
                    help='path to save log and model')
    ...
    log_name = args.load_ckpt.split('/')[-1]+'_'+str(args.input_pt_num)+'_'+str(args.img_thres)+'_'+str(args.pc_thres) +'_log.txt'
    # print(log_name)
    
    file_handler = logging.FileHandler('%s/%s' % (logdir,log_name))
    ...
    if args.mode == 'vis_registration':
        ...
        with torch.no_grad():
            t_diff,r_diff=test_acc_trans(model,testloader,args,logger)

# CUDA_VISIBLE_DEVICES=0 python run_kitti_360.py --load_ckpt='./res/360funsion_iter30_lr0.002/mode_last.t7'
# CUDA_VISIBLE_DEVICES=1 python run_kitti_360.py --load_ckpt='./res/360funsion_iter30_lr0.002/mode_epoch_29.t7'
# CUDA_VISIBLE_DEVICES=2 python run_kitti_360.py --load_ckpt='./res/360funsion_iter30_lr0.002/mode_epoch_28.t7'
# CUDA_VISIBLE_DEVICES=3 python run_kitti_360.py --load_ckpt='./res/360funsion_iter30_lr0.002/mode_epoch_27.t7'
# CUDA_VISIBLE_DEVICES=0 python run_kitti_360.py --mode='vis_registration' --load_ckpt='./res/360funsion_iter30_lr0.002/mode_last.t7'

# CUDA_VISIBLE_DEVICES=0 python run_kitti_360.py
# CUDA_VISIBLE_DEVICES=0 python run_kitti_360.py --mode='vis_registration'
# CUDA_VISIBLE_DEVICES=1 python run_kitti_360.py --load_ckpt='./kitti_res/360funsion_iter25_lr0.002_input_pt_num_20480/mode_last.t7'
# CUDA_VISIBLE_DEVICES=1 python run_kitti_360.py --mode='vis_registration' --load_ckpt='./kitti_res/360funsion_iter25_lr0.002_input_pt_num_20480/mode_last.t7'

# CUDA_VISIBLE_DEVICES=0 python run_kitti_360.py --exp_name='360funsion_iter25_lr0.001_input_pt_num_40960' --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_40960/mode_last.t7' --img_thres=0.95 --pc_thres=0.97
# CUDA_VISIBLE_DEVICES=1 python run_kitti_360.py --exp_name='360funsion_iter25_lr0.001_input_pt_num_40960' --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_40960/mode_last.t7' --img_thres=0.97 --pc_thres=0.95
# CUDA_VISIBLE_DEVICES=2 python run_kitti_360.py --exp_name='360funsion_iter25_lr0.001_input_pt_num_40960' --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_40960/mode_last.t7' --img_thres=0.95 --pc_thres=0.93
# CUDA_VISIBLE_DEVICES=3 python run_kitti_360.py --exp_name='360funsion_iter25_lr0.001_input_pt_num_40960' --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_40960/mode_last.t7' --img_thres=0.93 --pc_thres=0.95

# CUDA_VISIBLE_DEVICES=2 python run_kitti_360.py --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_40960/mode_last.t7' --img_thres=0.93 --pc_thres=0.93 
# CUDA_VISIBLE_DEVICES=3 python run_kitti_360.py --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_40960/mode_last.t7' --img_thres=0.97 --pc_thres=0.97 

# CUDA_VISIBLE_DEVICES=2 python run_kitti_360.py --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_61440/mode_epoch_24.t7' --input_pt_num=61440
# CUDA_VISIBLE_DEVICES=3 python run_kitti_360.py --load_ckpt='./kitti_res/360funsion_iter25_lr0.001_input_pt_num_61440/mode_last.t7' --input_pt_num=61440
