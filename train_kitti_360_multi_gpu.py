...
def get_parse():
    ...
    parser.add_argument('--save_path', type=str, default='./kitti_res', metavar='save_path',
    ...
    #                     help='num of CPUs')
    # parser.add_argument('--input_pt_num', type=int, default=61440, metavar='num_workers',
    #                     help='num of CPUs')
...
if __name__=='__main__':
    ...
    corr_loss_method = ContrastiveCorrelationLoss()
    ...
    for epoch in range(start_epoch, args.epoch):
        for step,data in enumerate(trainloader):
            ...
            img_features,pc_features,img_score,pc_score,pre_dist=model(pc,intensity,sn,img,pcd_360_img,gt_dist)    #64 channels feature
            ...
            if loss_reg.item() < 8:
                loss_all= loss_desc+loss_det+loss_epro+loss_reg
            else:
                loss_all= loss_desc+loss_det
            ...
            if global_step%6==0 and args.rank == 0:
                ...
                # logger.info('%s-%d-%d, loss: %f, loss desc: %f, loss det: %f, loss epro: %f, loss reg: %f, loss dist: %f'%('train',epoch,global_step,loss_all.data.cpu().numpy(),loss_desc.data.cpu().numpy(),loss_det.data.cpu().numpy(), loss_epro.data.cpu().numpy(), loss_reg.data.cpu().numpy(),loss_dist.cpu().numpy()))
                # print('%s-%d-%d, loss: %f, loss desc: %f, loss det: %f, loss epro: %f, loss reg: %f, loss dist: %f'%('train',epoch,global_step,loss_all.data.cpu().numpy(),loss_desc.data.cpu().numpy(),loss_det.data.cpu().numpy(), loss_epro.data.cpu().numpy(), loss_reg.data.cpu().numpy(),loss_dist.cpu().numpy()))
...
# python -m torch.distributed.launch --nproc_per_node=4 --use_env train_kitti_360_multi_gpu.py
# python -m torch.distributed.launch --nproc_per_node=4 --use_env train_kitti_360_multi_gpu.py --load_ckpt='mode_epoch_8.t7'
# python -m torch.distributed.launch --nproc_per_node=4 --use_env train_kitti_360_multi_gpu.py --load_ckpt='mode_epoch_16.t7'